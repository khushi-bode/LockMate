...
class PasswordGeneratorScreen(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        pw_frame = ctk.CTkFrame(
            self.card, fg_color=("gray85", "gray20"), corner_radius=8
        )
        ...
        self.copy_btn = ctk.CTkButton(
            pw_frame, text="Copy", width=70, command=self.copy_to_clipboard
        )
        ...
        self.strength_bar = ctk.CTkProgressBar(
            strength_frame, height=12, corner_radius=6
        )
        self.strength_bar.set(0)
        self.strength_bar.grid(row=0, column=0, sticky="ew", padx=(0, 10))

        self.strength_label = ctk.CTkLabel(
            strength_frame, text="", width=70, anchor="e"
        )
        ...
        ctk.CTkLabel(slider_frame, text="Password Length", anchor="w").grid(
            row=0, column=0, sticky="w"
        )

        self.length_value_label = ctk.CTkLabel(
            slider_frame, text="16", width=30, anchor="e"
        )
        self.length_value_label.grid(row=0, column=1)

        self.length_slider = ctk.CTkSlider(
            self.card, from_=8, to=64, number_of_steps=56, command=self.on_slider_change
        )
        ...
        ctk.CTkLabel(
            options_frame, text="Character Options", font=ctk.CTkFont(weight="bold")
        ...
        for text, var in checks:
            ctk.CTkCheckBox(
                options_frame, text=text, variable=var, command=self.generate
        ...
        ctk.CTkButton(btn_frame, text="Save to Vault", command=self.save_to_vault).pack(
            side="left", expand=True, padx=(10, 0), fill="x"
        )
