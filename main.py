...
class App(ctk.CTk):
    ...
    def reset_inactivity_timer(self, event=None):
        ...
        if hasattr(self.current_frame, "session_data") and getattr(
            self.current_frame, "session_data", None
        ):
    ...
    def show_dashboard_screen(self, session_data: dict):
        ...
        # Apply user's saved theme mode
        try:
            from app.data.user_repository import UserRepository

            repo = UserRepository()
            settings = repo.get_user_settings(session_data["user_id"])
            theme = settings.get("theme_mode", "System")
            ctk.set_appearance_mode(theme)
        except Exception as e:
            logger.error(f"Failed to load user theme: {e}")
